...
const availableCategories = computed(() => {
  ...
  if (
    ...
    !list.includes(editing.value.category)
  ) {
...
function validate() {
  ...
  if (!p) formErrors.percent = "Chegirma foizini kiriting.";
    ...
    formErrors.percent = "Foiz 1 dan 100 gacha bo'lishi kerak.";
  if (
    form.min_purchase !== null &&
    form.min_purchase !== "" &&
    Number(form.min_purchase) < 0
  )
...
async function save() {
  ...
  try {
    ...
    if (editing.value) {
      ...
      toast.success(
        "Tahrirlash so'rovi adminga yuborildi — tasdiqlanishini kuting.",
      );
    } else {
      await discountApi.create(payload);
      toast.success(
        "So'rov adminga yuborildi — tasdiqlangach chegirma qo'shiladi.",
      );
    }
  ...
  <DashboardLayout>
      ...
      <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <span class="shrink-0 text-2xl font-bold text-success"
              >{{ d.percent }}%</span
            >
          </div>
          ...
          <div class="mt-auto flex items-center justify-between pt-4">
            ...
            >
              ...
              <span
                ...
                :class="d.is_active ? 'text-success' : 'text-muted'"
              >
    ...
    >
      <form class="space-y-3" @submit.prevent="save">
        ...
        >
          ...
          <span
            >Chegirma to'g'ridan-to'g'ri qo'shilmaydi — avval adminга so'rov
            boradi, tasdiqlangach kuchga kiradi.</span
          >
        </div>
    ...
    <AppModal :open="!!deleteTarget" title="" @close="deleteTarget = null">
      <div v-if="deleteTarget" class="text-center">
        ...
        <p class="mt-1 text-xs text-muted">
          Quyidagi chegirma butunlay o'chiriladi<br />va bu amalni qaytarib
          bo'lmaydi.
        </p>
        ...
        <div class="mt-4 rounded-xl bg-secondary p-4 text-left">
          <div class="flex items-start justify-between gap-3">
            ...
            <span class="shrink-0 text-xl font-bold text-success"
              >{{ deleteTarget.percent }}%</span
            >
          </div>
          <p class="mt-2 text-xs text-muted">
            {{ deleteTarget.usage_count }} marta ishlatilgan
          </p>
        </div>
